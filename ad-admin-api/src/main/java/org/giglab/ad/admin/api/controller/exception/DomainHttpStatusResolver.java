...
public class DomainHttpStatusResolver {
  ...
  public static HttpStatus resolve(DomainErrorCode errorCode) {
    ...
    if (numeric >= 4000 && numeric < 4100) return HttpStatus.BAD_REQUEST;
    if (numeric >= 4100 && numeric < 4200) return HttpStatus.UNAUTHORIZED;
    if (numeric >= 4200 && numeric < 4300) return HttpStatus.CONFLICT;
    if (numeric >= 4300 && numeric < 4400) return HttpStatus.FORBIDDEN;
    if (numeric >= 4400 && numeric < 4500) return HttpStatus.NOT_FOUND;
    if (numeric >= 5000 && numeric < 6000) return HttpStatus.INTERNAL_SERVER_ERROR;
