...
| 템플릿 | 접두어 후보 | 예시 |
|--------|-----------|------|
| `bug_report` | `bug`, `fix`, `hotfix` | `[bug] 광고 조회 시 NPE 발생` |
| `feature` | `feat` | `[feat] 광고 캠페인 CRUD API 구현` |
| `refactoring` | `refactor`, `cleanup`, `perf` | `[refactor] 광고 도메인 패키지 구조 분리` |
| `task` | `docs`, `chore`, `config`, `ci` | `[chore] Spring Boot 의존성 추가` |
...
이슈를 생성하기 전에 **반드시** 아래 내용을 사용자에게 보여주고 확인을 받습니다:
- 제목 (접두어 포함)
- 본문 전체 (마크다운 형식)
- 라벨

사용자가 승인하면 다음 단계로 진행하고, 수정 요청이 있으면 반영 후 다시 확인합니다.
